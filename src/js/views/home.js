...
export const Home = () => {
	...
	const handleDelete = (el) => { console.log(el) }
	...
	const handleEdit = (e, el, i, method) => {
		e.preventDefault()
		console.log(el, method)
		method == "put" ? actions.updateTask(el, i) : actions.deleteTask(el.id)
	}
					...
					{store.todos?.length > 0 ?
						(<>
							{store.todos?.map((el, i) =>
								<div key={i} className="input-group flex-nowrap d-flex mt-1">
									<input type="text" className="form-control border-0" defaultValue={el.label} onChange={(e) => el.label = e.target.value} />
									<button className="btn btn-light " onClick={e => handleEdit(e, el, i, "put")} ><span className="fa-regular fa-pen-to-square text-success"></span></button>
									<button className="btn btn-light " onClick={e => handleEdit(e, el, i, "delete")}><span className="fa-regular fa-trash-can text-danger"></span></button>
								</div>
							)}
